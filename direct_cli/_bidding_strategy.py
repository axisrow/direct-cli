...
class _TextStrategyConfig(NamedTuple):
    """Per-side (search/network) configuration for the shared TextCampaign
    bidding-strategy core. Bundles the constants, flag prefixes and error
    labels that differ between ``TextCampaign.BiddingStrategy.Search`` and
    ``...Network`` so the ~280-line validation+assembly tail can live once.

    ``field_support`` maps each typed flag to the subtype-set constant that
    declares it; the remaining members mirror the per-side ``_TEXT_SEARCH_*``
    / ``_TEXT_NETWORK_*`` constants used in the original two builders.
    """

    side_label: str  # "Search" / "Network" (error text)
    # Budget-block "is not valid" label: search historically omits the side
    # word ("TextCampaign ..."), network includes it ("TextCampaign Network").
    budget_invalid_label: str
    flag_prefix: str  # "--text-search-" / "--text-network-"
    strategy_flag: str  # "--search-strategy" / "--network-strategy"
    strategy_value: Optional[str]  # raw search_strategy / network_strategy
    budget_type_enum: List[str]
    custom_period_subtypes: Set[str]
    exploration_budget_subtypes: Set[str]
    requires_priority_goals: Set[str]
    min_priority_goals: Dict[str, int]
    required_typed_flags: Dict[str, Dict[str, str]]
    required_typed_flags_update: Dict[str, Dict[str, str]]
    # Search rejects WeeklyClickPackage + (average_cpc & bid_ceiling); the
    # network builder never had this mutex, so it stays search-only.
    weekly_click_package_mutex: bool


def _build_text_strategy_block(
    *,
    container: dict,
    subtype: str,
    normalized_strategy: str,
    config: _TextStrategyConfig,
    field_support: Dict[str, tuple],
    ...
    """Shared validation + Strategy*Add block assembly for a TextCampaign
    search/network subtype. ``container`` already holds ``BiddingStrategyType``
    (and, for search, placement fields); this mutates it in place with the
    subtype block and returns it. Byte-for-byte identical to the original
    inline tails of the two builders, parameterised by ``config``.
    """
    side = config.side_label
    ...
    for flag, (value, supported) in field_support.items():
        if value is not None and subtype not in supported:
            raise click.UsageError(
                f"{flag} is not valid for TextCampaign {side} strategy "
    ...
    if reserve_return is not None and reserve_return % 10 != 0:
        raise click.UsageError(
            f"{prefix}reserve-return must be a multiple of 10 "
            "(0-100), per Yandex Direct API docs"
        )

    # LimitPercent doc constraint: 10-100 by tens (network NetworkDefault only;
    # search never supplies limit_percent so this branch is inert there).
    if limit_percent is not None and (
        limit_percent < 10 or limit_percent > 100 or limit_percent % 10 != 0
    ):
        raise click.UsageError(
            f"{prefix}limit-percent must be a multiple of 10 from 10 to 100"
        )

    if custom_period is not None and subtype not in config.custom_period_subtypes:
        raise click.UsageError(
            f"{config.budget_invalid_label} CustomPeriodBudget is not valid "
            f"for {normalized_strategy}"
        )
    if (
        exploration_budget is not None
        and subtype not in config.exploration_budget_subtypes
    ):
        raise click.UsageError(
            f"{config.budget_invalid_label} ExplorationBudget is not valid "
            f"for {normalized_strategy}"
        )
    ...
    if weekly_spend_limit is not None and custom_period is not None:
        raise click.UsageError(
            f"{prefix}weekly-spend-limit cannot be combined with "
            f"{prefix}custom-period-spend-limit"
        )
    ...
    if budget_type is not None:
        if not is_update:
            raise click.UsageError(f"{prefix}budget-type is update-only")
        normalized_budget_type = budget_type.upper()
        if normalized_budget_type not in config.budget_type_enum:
            raise click.UsageError(
                f"{prefix}budget-type must be one of "
                f"{', '.join(config.budget_type_enum)}"
            )
        if normalized_budget_type == "CUSTOM_PERIOD_BUDGET" and custom_period is None:
            raise click.UsageError(
                f"{prefix}budget-type CUSTOM_PERIOD_BUDGET requires the "
                "full CustomPeriodBudget flag set"
            )
        if normalized_budget_type == "WEEKLY_BUDGET" and weekly_spend_limit is None:
            raise click.UsageError(
                f"{prefix}budget-type WEEKLY_BUDGET requires "
                f"{prefix}weekly-spend-limit"
            )
    ...
    # Search-only — the network builder historically omitted this mutex.
    if (
        config.weekly_click_package_mutex
        and subtype == "WeeklyClickPackage"
        and average_cpc is not None
        and bid_ceiling is not None
    ):
        raise click.UsageError(
            f"WEEKLY_CLICK_PACKAGE cannot combine {prefix}average-cpc "
            "with --bid-ceiling"
        )
    ...
    # is a sibling of BiddingStrategy, NOT nested inside it). On update the
    # caller has already placed PriorityGoals via the dedicated
    # ``PriorityGoalsUpdateSetting`` shape, so we only validate here.
    if priority_goals_items is not None:
        if subtype not in config.requires_priority_goals:
            raise click.UsageError(
                "--priority-goals is only valid with "
                "AVERAGE_CPA_MULTIPLE_GOALS / "
                "PAY_FOR_CONVERSION_MULTIPLE_GOALS / MAX_PROFIT strategies; "
                f"got {config.strategy_flag}={config.strategy_value!r}"
            )
        min_required = config.min_priority_goals.get(subtype)
        if min_required is not None and len(priority_goals_items) < min_required:
            raise click.UsageError(
                ...
                f"for {config.strategy_value} per Yandex Direct API docs"
            )
        if not is_update:
            sub_campaign_block["PriorityGoals"] = {"Items": priority_goals_items}

    # WSDL minOccurs=1 fields — fail-fast on add. On update the API tolerates
    # partial blocks; when switching the strategy type the conceptually
    # required fields for the new subtype must still be supplied.
    ...
    if not is_update:
        required = config.required_typed_flags.get(subtype, {})
        missing = [
            flag
            for wsdl_field, flag in required.items()
            if provided_lookup.get(wsdl_field) is None
    ...
    elif config.strategy_value is not None:
        required = config.required_typed_flags_update.get(subtype, {})
        missing = [
            flag
            for wsdl_field, flag in required.items()
            if provided_lookup.get(wsdl_field) is None
        ...
        if missing:
            raise click.UsageError(
                ...
                f"{config.strategy_flag} on update (per Yandex Direct "
                "API docs)"
            )

    # Build the WSDL Strategy*Add block. Element order follows WSDL sequence
    # order for readability — JSON-RPC does not require ordering.
    block: dict = {}
    ...
    # numeric fields are set — PriorityGoals lives on the parent block and the
    # subtype block is the only signal the API uses to discriminate the
    # strategy on add.
    if block or subtype in config.requires_priority_goals:
...
def build_text_campaign_search_strategy(
    ...
    *_MULTIPLE_GOALS strategies (WSDL ``TextCampaignAddItem.PriorityGoals``
    ...
    config = _TextStrategyConfig(
        side_label="Search",
        budget_invalid_label="TextCampaign",
        flag_prefix="--text-search-",
        strategy_flag="--search-strategy",
        strategy_value=search_strategy,
        budget_type_enum=_TEXT_CAMPAIGN_SEARCH_BUDGET_TYPES,
        custom_period_subtypes=_TEXT_SEARCH_SUPPORTS_CUSTOM_PERIOD_BUDGET,
        exploration_budget_subtypes=_TEXT_SEARCH_SUPPORTS_EXPLORATION_BUDGET,
        requires_priority_goals=_TEXT_SEARCH_REQUIRES_PRIORITY_GOALS,
        min_priority_goals=_TEXT_SEARCH_MIN_PRIORITY_GOALS,
        required_typed_flags=_TEXT_SEARCH_REQUIRED_TYPED_FLAGS,
        required_typed_flags_update=_TEXT_SEARCH_REQUIRED_TYPED_FLAGS_UPDATE,
        weekly_click_package_mutex=True,
    )
    return _build_text_strategy_block(
        ...
        sub_campaign_block=sub_campaign_block,
    )
...
class _DynamicTextStrategyConfig(NamedTuple):
    """Per-side (search/network) configuration for the shared
    DynamicTextCampaign bidding-strategy core. Bundles the flag prefix,
    error labels and per-flag subtype-support sets that differ between
    ``DynamicTextCampaign.BiddingStrategy.Search`` and ``...Network`` so
    the shared validation+assembly pipeline can live once.
    """

    side_label: str  # "" (search omits the side word) / "Search" / "Network"
    flag_prefix: str  # "--dyn-search-" / "--dyn-network-"
    strategy_flag: str  # "--search-strategy" / "--network-strategy"
    strategy_value: Optional[str]
    weekly_spend_limit_subtypes: Set[str]
    bid_ceiling_subtypes: Set[str]
    custom_period_subtypes: Set[str]
    goal_id_subtypes: Set[str]
    average_cpa_subtypes: Set[str]
    cpa_subtypes: Set[str]
    average_cpc_subtypes: Set[str]
    crr_subtypes: Set[str]
    clicks_per_week_subtypes: Set[str]
    reserve_return_subtypes: Set[str]
    roi_coef_subtypes: Set[str]
    profitability_subtypes: Set[str]
    exploration_budget_subtypes: Set[str]
    budget_type_subtypes: Set[str]
    # limit_percent is network-only; search config passes empty sets and
    # the caller never supplies a non-None value on that side.
    limit_percent_subtypes: Set[str]
    required_map: Dict[str, List[str]]
    # Whether BudgetType presence is cross-checked against the budget slice
    # (CUSTOM_PERIOD_BUDGET requires the full CustomPeriodBudget flag set,
    # WEEKLY_BUDGET requires weekly-spend-limit). Network enforces this up
    # front; search intentionally surfaces the wire-level round-trip error
    # instead of inventing a tighter local contract than the WSDL declares
    # (#362 adversarial-review feedback), so search passes ``False`` here.
    budget_type_presence_checks: bool


def _build_dynamic_text_strategy_block(
    *,
    container: dict,
    subtype: str,
    normalized_strategy: str,
    config: _DynamicTextStrategyConfig,
    limit_percent: Optional[int],
    reserve_return: Optional[int],
    roi_coef: Optional[int],
    average_cpa: Optional[int],
    ...
    """Shared validation + Strategy*Add block assembly for a
    DynamicTextCampaign search/network subtype. ``container`` already holds
    ``BiddingStrategyType`` (and, for search, ``PlacementTypes``); this
    mutates it in place with the subtype block and returns it. Byte-for-byte
    identical to the original inline tails of the two builders, parameterised
    by ``config``.
    """
    prefix = config.flag_prefix
    side_label = f"{config.side_label} " if config.side_label else ""
    ...
    exploration_provided = [
        flag
        for flag, value in (
            (f"{prefix}exploration-budget", exploration_budget),
            (f"{prefix}exploration-budget-custom", exploration_budget_custom),
        )
        if value is not None
    ]
    if exploration_provided and len(exploration_provided) != 2:
        missing = (
            f"{prefix}exploration-budget-custom"
            if exploration_budget is not None
            else f"{prefix}exploration-budget"
        )
        raise click.UsageError(
            f"DynamicTextCampaign {side_label}ExplorationBudget requires both "
            f"{prefix}exploration-budget and "
            f"{prefix}exploration-budget-custom; missing {missing}"
        )
    if exploration_provided and subtype not in config.exploration_budget_subtypes:
        raise click.UsageError(
            f"{normalized_strategy} does not accept "
            f"{prefix}exploration-budget / "
            f"{prefix}exploration-budget-custom"
        )
    ...
    custom_period_values = {
        f"{prefix}custom-period-spend-limit": custom_period_spend_limit,
        f"{prefix}custom-period-start-date": custom_period_start_date,
        f"{prefix}custom-period-end-date": custom_period_end_date,
        f"{prefix}custom-period-auto-continue": custom_period_auto_continue,
    }
    custom_period_flags = [
        flag for flag, value in custom_period_values.items() if value is not None
    ]
    if custom_period_flags and len(custom_period_flags) != len(custom_period_values):
        ...
        raise click.UsageError(
            f"DynamicTextCampaign {side_label}CustomPeriodBudget requires all "
            f"custom-period flags; missing {', '.join(sorted(missing))}"
        )
    if custom_period_flags and subtype not in config.custom_period_subtypes:
        raise click.UsageError(
            f"{normalized_strategy} does not accept DynamicTextCampaign "
            f"{side_label}CustomPeriodBudget flags"
        )
    if weekly_spend_limit is not None and custom_period_flags:
        raise click.UsageError(
            f"{prefix}weekly-spend-limit cannot be combined with "
    ...
    if limit_percent is not None:
        ...
        if subtype not in config.limit_percent_subtypes:
            raise click.UsageError(
                f"{normalized_strategy} does not accept {prefix}limit-percent"
            )
    ...
    if not is_update:
        ...
        flag_lookup = {
            "AverageCpc": f"{prefix}average-cpc",
            "AverageCpa": f"{prefix}average-cpa",
            "Cpa": f"{prefix}cpa",
            "GoalId": f"{prefix}goal-id",
            "Crr": f"{prefix}crr",
            "ClicksPerWeek": f"{prefix}clicks-per-week",
            "ReserveReturn": f"{prefix}reserve-return",
            "RoiCoef": f"{prefix}roi-coef",
        }
        required_fields = config.required_map.get(subtype, [])
        missing = [
            flag_lookup[wsdl_field]
            for wsdl_field in required_fields
            if provided_lookup.get(wsdl_field) is None
        ]
        if missing:
            raise click.UsageError(
    ...
    if budget_type is not None:
        if not is_update:
            raise click.UsageError(f"{prefix}budget-type is update-only")
        if subtype not in config.budget_type_subtypes:
            raise click.UsageError(
                f"{normalized_strategy} does not accept {prefix}budget-type"
            )
        if config.budget_type_presence_checks:
            normalized_budget_type = budget_type.upper()
            if (
                normalized_budget_type == "CUSTOM_PERIOD_BUDGET"
                and not custom_period_flags
            ):
                raise click.UsageError(
                    f"{prefix}budget-type CUSTOM_PERIOD_BUDGET requires "
                    "full CustomPeriodBudget flags"
                )
            if normalized_budget_type == "WEEKLY_BUDGET" and weekly_spend_limit is None:
                raise click.UsageError(
    ...
    field_support = {
        f"{prefix}weekly-spend-limit": (
            weekly_spend_limit,
            config.weekly_spend_limit_subtypes,
        ),
        f"{prefix}bid-ceiling": (bid_ceiling, config.bid_ceiling_subtypes),
        f"{prefix}average-cpc": (average_cpc, config.average_cpc_subtypes),
        f"{prefix}average-cpa": (average_cpa, config.average_cpa_subtypes),
        f"{prefix}cpa": (cpa, config.cpa_subtypes),
        f"{prefix}goal-id": (goal_id, config.goal_id_subtypes),
        f"{prefix}crr": (crr, config.crr_subtypes),
        f"{prefix}clicks-per-week": (
            clicks_per_week,
            config.clicks_per_week_subtypes,
        ),
        f"{prefix}reserve-return": (
            reserve_return,
            config.reserve_return_subtypes,
        ),
        f"{prefix}roi-coef": (roi_coef, config.roi_coef_subtypes),
        f"{prefix}profitability": (
            profitability,
            config.profitability_subtypes,
        ),
    }
    for flag, (value, supported_subtypes) in field_support.items():
        if value is not None and subtype not in supported_subtypes:
    ...
    # CustomPeriodBudget / ExplorationBudget block construction is delegated
    # to the shared TextCampaign-precedent helpers; the full-or-none checks
    # above already guarantee either all four / both fields are present or
    # none are, so neither helper's own error branch can trigger here.
    custom_period = _build_custom_period_budget(
        f"{prefix}custom-period-",
        "DynamicTextCampaign",
        custom_period_spend_limit,
        custom_period_start_date,
        custom_period_end_date,
    ...
    exploration = _build_exploration_budget(
        f"{prefix}exploration-",
        "DynamicTextCampaign",
        exploration_budget,
        exploration_budget_custom,
        exploration_yes_only=False,
    )

    # Build the WSDL Strategy*Add block. Element order follows the original
    # inline assembly order for readability — JSON-RPC does not require
    # ordering.
    block: dict = {}
    if limit_percent is not None:
        block["LimitPercent"] = limit_percent
    ...
    if exploration is not None:
        block["ExplorationBudget"] = exploration
    if budget_type is not None:
        normalized_budget_type = budget_type.upper()
...
def build_dynamic_text_network_strategy(
    ...
    *,
    ...
    config = _DynamicTextStrategyConfig(
        side_label="",
        flag_prefix="--dyn-network-",
        strategy_flag="--network-strategy",
        strategy_value=network_strategy,
        weekly_spend_limit_subtypes=_DYN_NETWORK_WEEKLY_SPEND_LIMIT_SUBTYPES,
        bid_ceiling_subtypes=_DYN_NETWORK_BID_CEILING_SUBTYPES,
        custom_period_subtypes=_DYN_NETWORK_CUSTOM_PERIOD_SUBTYPES,
        goal_id_subtypes=_DYN_NETWORK_GOAL_ID_SUBTYPES,
        average_cpa_subtypes=_DYN_NETWORK_AVERAGE_CPA_SUBTYPES,
        cpa_subtypes=_DYN_NETWORK_CPA_SUBTYPES,
        average_cpc_subtypes=_DYN_NETWORK_AVERAGE_CPC_SUBTYPES,
        crr_subtypes=_DYN_NETWORK_CRR_SUBTYPES,
        clicks_per_week_subtypes=_DYN_NETWORK_CLICKS_PER_WEEK_SUBTYPES,
        reserve_return_subtypes=_DYN_NETWORK_RESERVE_RETURN_SUBTYPES,
        roi_coef_subtypes=_DYN_NETWORK_ROI_COEF_SUBTYPES,
        profitability_subtypes=_DYN_NETWORK_PROFITABILITY_SUBTYPES,
        exploration_budget_subtypes=_DYN_NETWORK_EXPLORATION_BUDGET_SUBTYPES,
        budget_type_subtypes=_DYN_NETWORK_BUDGET_TYPE_SUBTYPES,
        limit_percent_subtypes=_DYN_NETWORK_LIMIT_PERCENT_SUBTYPES,
        budget_type_presence_checks=True,
        ...
        required_map={
            "WbMaximumClicks": [],
            "WbMaximumConversionRate": ["GoalId"],
            ...
            "WeeklyClickPackage": ["ClicksPerWeek"],
            "NetworkDefault": [],
        },
    )
    return _build_dynamic_text_strategy_block(
        container=network,
        subtype=subtype,
        normalized_strategy=normalized_strategy,
        config=config,
...
def build_dynamic_text_search_strategy(
    ...
    *,
    ...
    config = _DynamicTextStrategyConfig(
        side_label="Search",
        flag_prefix="--dyn-search-",
        strategy_flag="--search-strategy",
        strategy_value=search_strategy,
        weekly_spend_limit_subtypes=_DYN_SEARCH_WEEKLY_SPEND_LIMIT_SUBTYPES,
        bid_ceiling_subtypes=_DYN_SEARCH_BID_CEILING_SUBTYPES,
        custom_period_subtypes=_DYN_SEARCH_CUSTOM_PERIOD_SUBTYPES,
        goal_id_subtypes=_DYN_SEARCH_GOAL_ID_SUBTYPES,
        average_cpa_subtypes=_DYN_SEARCH_AVERAGE_CPA_SUBTYPES,
        cpa_subtypes=_DYN_SEARCH_CPA_SUBTYPES,
        average_cpc_subtypes=_DYN_SEARCH_AVERAGE_CPC_SUBTYPES,
        crr_subtypes=_DYN_SEARCH_CRR_SUBTYPES,
        clicks_per_week_subtypes=_DYN_SEARCH_CLICKS_PER_WEEK_SUBTYPES,
        reserve_return_subtypes=_DYN_SEARCH_RESERVE_RETURN_SUBTYPES,
        roi_coef_subtypes=_DYN_SEARCH_ROI_COEF_SUBTYPES,
        profitability_subtypes=_DYN_SEARCH_PROFITABILITY_SUBTYPES,
        exploration_budget_subtypes=_DYN_SEARCH_EXPLORATION_BUDGET_SUBTYPES,
        budget_type_subtypes=_DYN_SEARCH_BUDGET_TYPE_SUBTYPES,
        limit_percent_subtypes=set(),
        budget_type_presence_checks=False,
        required_map={
            "WbMaximumClicks": [],
            "WbMaximumConversionRate": ["GoalId"],
            "AverageCpc": ["AverageCpc"],
...
class _SmartStrategyConfig(NamedTuple):
    """Per-side (search/network) configuration for the shared SmartCampaign
    bidding-strategy core. Bundles the flag prefix, allowed strategy list,
    strategy→subtype map, default strategy and per-side field/required-field
    tables that differ between ``SmartCampaign.BiddingStrategy.Search`` and
    ``...Network`` so the shared validation+assembly pipeline can live once,
    mirroring the ``_TextStrategyConfig`` / ``_DynamicTextStrategyConfig``
    precedent (#581, #592).
    """

    side_label: str  # "Search" / "Network" (error text)
    flag_prefix: str  # "--smart-search-" / "--smart-network-"
    strategy_flag: str  # "--search-strategy" / "--network-strategy"
    default_strategy: str  # "SERVING_OFF" / "AVERAGE_CPC_PER_FILTER"
    allowed_strategies: List[str]
    strategy_to_subtype: Dict[str, str]
    required_fields: Dict[str, List[tuple]]
    field_support: Dict[str, set]
    # LimitPercent is network-only; search config passes False and the
    # caller never supplies a non-None value on that side.
    has_limit_percent: bool


def _build_smart_strategy_block(
    *,
    ...
    budget_type: Optional[str],
    config: _SmartStrategyConfig,
    include_default: bool,
    is_update: bool,
) -> Optional[dict]:
    """Shared validation + Strategy*Add block assembly for a SmartCampaign
    search/network side, parameterised by ``config``. Byte-for-byte identical
    to the original inline bodies of the two builders.
    """
    prefix = config.flag_prefix
    detail_values = {
        f"{prefix}average-cpc": average_cpc,
        f"{prefix}filter-average-cpc": filter_average_cpc,
        f"{prefix}average-cpa": average_cpa,
        f"{prefix}filter-average-cpa": filter_average_cpa,
        f"{prefix}cpa": cpa,
        f"{prefix}goal-id": goal_id,
        f"{prefix}weekly-spend-limit": weekly_spend_limit,
        f"{prefix}bid-ceiling": bid_ceiling,
        f"{prefix}reserve-return": reserve_return,
        f"{prefix}roi-coef": roi_coef,
        f"{prefix}profitability": profitability,
        f"{prefix}crr": crr,
        ...
        f"{prefix}budget-type": budget_type,
    }
    if config.has_limit_percent:
        detail_values[f"{prefix}limit-percent"] = limit_percent
    has_details = any(value is not None for value in detail_values.values())
    if not include_default and strategy is None:
        if has_details:
            raise click.UsageError(
                f"SmartCampaign {config.side_label.lower()} detail flags "
                f"require {config.strategy_flag}"
            )
        return None
    if has_details and strategy is None:
        raise click.UsageError(
            f"SmartCampaign {config.side_label.lower()} detail flags require "
            f"{config.strategy_flag}"
        )

    normalized_strategy = (strategy or config.default_strategy).upper()
    if normalized_strategy not in config.allowed_strategies:
        raise click.UsageError(
            f"{config.strategy_flag} for SMART_CAMPAIGN must be one of "
            f"{', '.join(config.allowed_strategies)}"
        )

    subtype = config.strategy_to_subtype.get(normalized_strategy)
    container: dict = {"BiddingStrategyType": normalized_strategy}
    if subtype is None:
        # SERVING_OFF carries no nested block — every typed flag is invalid.
        invalid = [flag for flag, value in detail_values.items() if value is not None]
        if invalid:
            raise click.UsageError(
                f"{normalized_strategy} does not accept SmartCampaign "
                f"{config.side_label.lower()} detail flags: "
                f"{', '.join(sorted(invalid))}"
            )
        return container
    ...
    if (
        ...
        and subtype not in config.field_support["CustomPeriodBudget"]
    ):
        raise click.UsageError(
            f"{normalized_strategy} does not accept SmartCampaign "
            f"{config.side_label} CustomPeriodBudget flags"
        )
    if weekly_spend_limit is not None and custom_period is not None:
        raise click.UsageError(
            f"{prefix}weekly-spend-limit cannot be combined with "
    ...
    exploration_budget_block = _build_exploration_budget(
        ...
        f"SmartCampaign {config.side_label}",
        exploration_min_budget,
        exploration_min_budget_custom,
        exploration_yes_only=False,
    )
    if (
        exploration_budget_block is not None
        and subtype not in config.field_support["ExplorationBudget"]
    ):
        raise click.UsageError(
            f"{normalized_strategy} does not accept SmartCampaign "
            f"{config.side_label} ExplorationBudget flags"
        )

    # LimitPercent: documented local CLI constraint (multiple of 10 in
    # 10..100), network-only.
    if config.has_limit_percent and limit_percent is not None:
        if limit_percent < 10 or limit_percent > 100 or limit_percent % 10 != 0:
            raise click.UsageError(
                f"{prefix}limit-percent must be a multiple of 10 " "from 10 to 100"
            )
        if subtype not in config.field_support["LimitPercent"]:
            raise click.UsageError(
                f"{normalized_strategy} does not accept {prefix}limit-percent"
            )
    ...
    if not is_update:
        required = config.required_fields.get(subtype, [])
        missing = []
        provided_lookup = {
            "average_cpc": average_cpc,
    ...
    field_flags = {
        f"{prefix}average-cpc": ("AverageCpc", average_cpc),
        f"{prefix}filter-average-cpc": ("FilterAverageCpc", filter_average_cpc),
        f"{prefix}average-cpa": ("AverageCpa", average_cpa),
        f"{prefix}filter-average-cpa": ("FilterAverageCpa", filter_average_cpa),
        f"{prefix}cpa": ("Cpa", cpa),
        f"{prefix}goal-id": ("GoalId", goal_id),
        f"{prefix}weekly-spend-limit": ("WeeklySpendLimit", weekly_spend_limit),
        f"{prefix}bid-ceiling": ("BidCeiling", bid_ceiling),
        f"{prefix}reserve-return": ("ReserveReturn", reserve_return),
        f"{prefix}roi-coef": ("RoiCoef", roi_coef),
        f"{prefix}profitability": ("Profitability", profitability),
        f"{prefix}crr": ("Crr", crr),
    }
    for flag, (wsdl_field, value) in field_flags.items():
        if value is not None and subtype not in config.field_support[wsdl_field]:
            raise click.UsageError(f"{normalized_strategy} does not accept {flag}")
    ...
    block = _build_smart_flat_block(
        limit_percent=limit_percent if config.has_limit_percent else None,
        average_cpc=average_cpc,
        filter_average_cpc=filter_average_cpc,
        average_cpa=average_cpa,
        filter_average_cpa=filter_average_cpa,
...
_SMART_SEARCH_STRATEGY_CONFIG = _SmartStrategyConfig(
    side_label="Search",
    flag_prefix="--smart-search-",
    strategy_flag="--search-strategy",
    default_strategy="SERVING_OFF",
    allowed_strategies=SMART_CAMPAIGN_SEARCH_STRATEGIES,
    strategy_to_subtype=SMART_CAMPAIGN_SEARCH_STRATEGY_TO_SUBTYPE,
    required_fields=SMART_CAMPAIGN_SEARCH_REQUIRED_FIELDS,
    field_support=SMART_CAMPAIGN_SEARCH_FIELD_SUPPORT,
    has_limit_percent=False,
)
...
def build_smart_campaign_search_strategy(
    ...
    *,
    ...
    """
    return _build_smart_strategy_block(
        ...
        budget_type=budget_type,
        config=_SMART_SEARCH_STRATEGY_CONFIG,
        include_default=include_default,
        is_update=is_update,
    )
...
def build_text_campaign_network_strategy(
    *,
    ...
    config = _TextStrategyConfig(
        side_label="Network",
        budget_invalid_label="TextCampaign Network",
        flag_prefix="--text-network-",
        strategy_flag="--network-strategy",
        strategy_value=network_strategy,
        budget_type_enum=BUDGET_TYPES,
        custom_period_subtypes=_TEXT_NETWORK_CUSTOM_PERIOD_SUBTYPES,
        exploration_budget_subtypes=_TEXT_NETWORK_EXPLORATION_BUDGET_SUBTYPES,
        requires_priority_goals=_TEXT_NETWORK_REQUIRES_PRIORITY_GOALS,
        min_priority_goals=_TEXT_NETWORK_MIN_PRIORITY_GOALS,
        required_typed_flags=_TEXT_NETWORK_REQUIRED_TYPED_FLAGS,
        required_typed_flags_update=_TEXT_NETWORK_REQUIRED_TYPED_FLAGS_UPDATE,
        weekly_click_package_mutex=False,
    )
    return _build_text_strategy_block(
        container=network,
        subtype=subtype,
...
_SMART_NETWORK_STRATEGY_CONFIG = _SmartStrategyConfig(
    side_label="Network",
    flag_prefix="--smart-network-",
    strategy_flag="--network-strategy",
    default_strategy="AVERAGE_CPC_PER_FILTER",
    allowed_strategies=SMART_CAMPAIGN_NETWORK_STRATEGIES,
    strategy_to_subtype=SMART_CAMPAIGN_NETWORK_STRATEGY_TO_SUBTYPE,
    required_fields=SMART_CAMPAIGN_NETWORK_REQUIRED_FIELDS,
    field_support=SMART_CAMPAIGN_NETWORK_FIELD_SUPPORT,
    has_limit_percent=True,
)
...
def build_smart_campaign_network_strategy(
    ...
    *,
    ...
    return _build_smart_strategy_block(
        ...
        exploration_min_budget_custom=exploration_min_budget_custom,
        budget_type=budget_type,
        config=_SMART_NETWORK_STRATEGY_CONFIG,
        include_default=include_default,
        is_update=is_update,
    )
