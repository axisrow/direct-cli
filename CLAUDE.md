...
**`direct masters` cookie decryption (#634).** On macOS, Chrome's cookie AES key lives *only* in the login Keychain (`Chrome Safe Storage`) — never in `Local State`, whose `os_crypt.encrypted_key` field is populated on Windows only. `direct_cli/browser/_chrome_crypto.py` reads the Keychain password itself, derives the AES-128-CBC key (PBKDF2-HMAC-SHA1, Chromium's `saltysalt`/1003-iteration constants), and decrypts only the Yandex Direct cookies it needs — it does not copy the user's Chrome profile or rely on a real Chrome to decrypt anything. `direct_cli/browser/session.py::open_chrome_session` injects the result via Playwright's `add_cookies()` into a bundled Chromium context. The same marker-registry pattern used for captchas (`_captcha.py`) covers Yandex's login-page markers (`session.py::assert_authenticated`), so an expired/wrong-account session is reported explicitly instead of surfacing as a `networkidle` timeout.

**`direct playwright login`/`doctor` — persisted browser session.** `open_chrome_session` above decrypts from Chrome on every call; `direct playwright login` (`direct_cli/commands/browser_session.py`) instead decrypts once via `session.py::capture_storage_state` and persists the result as a Playwright `storage_state` file (`direct_cli/browser/store.py`, `~/.direct-cli/playwright/session.json`, `0600` inside `0700`, sibling of `auth.py`'s `AUTH_STORE_PATH`). `direct masters`' `_open_session` (`commands/masters.py`) then prefers this saved session (`session.py::open_saved_session`) over a fresh Keychain decrypt, falling back automatically when no session is saved, an explicit `--profile-dir`/`--chrome-profile` was passed, or the saved session turns out to be stale (`BrowserAuthError` triggers one retry via fresh decrypt). `direct playwright doctor` (`direct_cli/browser/diagnostics.py`) is a **read-only** diagnostic over the whole chain (playwright/cryptography installed, chromium downloaded, Chrome profile found, Keychain key derivable, cookies decryptable, saved session present/fresh) — it never logs in, never launches a browser of its own, and never writes to disk; that invariant is enforced by contract tests in `tests/test_playwright_session.py`.

**`direct masters list` reads the grid's JSON API, not its DOM (#639).** Live diagnosis found the campaigns grid (`https://direct.yandex.ru/dna/grid/campaigns/`) is a virtualized SPA that never renders a `a[href*='/wizard/campaigns/']` anchor, so the original DOM-scraping approach could never have worked. `direct_cli/browser/masters.py::fetch_masters_list` instead intercepts the grid's own data call — `POST /web-api/grid/api?operationName=GridCampaigns` — via `page.on("response", ...)`, replays it through `page.request.post(...)` (with the captured headers) to paginate past its 200-row `GRID_PAGE_LIMIT`, and never hand-assembles the GraphQL query itself (it is several KB of fragments that would drift out of sync with Yandex's schema). Rows created via Мастер кампаний have no distinguishing `type`/`metaType`/`__typename` — the one field that does discriminate them, confirmed live, is `source == "UAC"` (`MASTERS_SOURCE`). `status.primaryStatus` is filtered client-side via `STATUS_FILTERS` (`masters list --status`, default `not-archived`) because the grid's `status-filter` URL query parameter is silently ignored server-side. `masters` has **no `--login`/agency support**: passing your own login as Yandex's managed-client `ulogin` parameter produces HTTP 401 "Доступ ограничен" on the grid's data calls (confirmed live) — both `list` and `get` build URLs without `ulogin`, and `masters` is in `cli.py`'s `_NO_CREDENTIALS_GROUPS`.
...
- **Browser poll loops use an injectable clock** (issue #767). Every `while <deadline>: … page.wait_for_timeout(tick)` loop in `direct_cli/browser/` reads its deadline from `direct_cli/browser/_clock.py::now()` (`time.monotonic` in production), *never* from `time.monotonic()` directly. Under the offline harness `FakePage.wait_for_timeout` is a no-op, so a real deadline can never be ticked past and the loop busy-spins for the full production timeout — that alone made `tests/test_masters.py` take ~880s sequentially (now ~1s). `tests/test_masters.py` installs a module-wide fake clock in `setUpModule` that advances only inside `wait_for_timeout`; a `FakePage` subclass overriding `wait_for_timeout` **must** still advance it (call `_advance_fake_clock` or `super()`), or its loop never terminates — a frozen clock makes the deadline unreachable, which is an infinite loop (measured: 15.8M iterations in 8s), not a slow test. `TestBrowserPackageClock` guards against reintroducing a raw clock deadline: `_raw_clock_calls` parses each module's AST and bans the `time`/`datetime` modules **wholesale** rather than denylisting function names — a `{monotonic, perf_counter}` denylist let `monotonic_ns`, `time_ns`, `process_time`, `datetime.now()`, aliased imports (`import time as _t`), assignment rebinds (`_t = time`) and `getattr(time, "monotonic")()` straight through, and `time.monotonic_ns()` was measured reintroducing the full regression (1.0s → 62s) with the guard reporting clean. The one carve-out is `_WALL_CLOCK_CARVE_OUTS`, scoped **per file** (`store.py` → `time.time`/`time_ns`), because `store.py` stamps calendar time into the session envelope, where monotonic would be wrong. Keep it per-file: allowing `time.time` package-wide reopened the whole bug for one commit — a wall clock is no more tickable by a no-op `wait_for_timeout` than a monotonic one, and `deadline = time.time() + …` measured 61.89s with the guard green. Backstop: `--timeout=60` in `pyproject.toml`'s `addopts` (pytest-timeout) turns any future frozen-clock loop into a fast failure instead of a stuck xdist worker. It belongs in `addopts`, not a top-level `timeout =` ini key — that key is global and would also cap the live tiers, whose tests create draft resources their own cleanup deletes, so a mid-test kill would orphan them on a production account. Add `--timeout=0` to a genuinely slow live run.
