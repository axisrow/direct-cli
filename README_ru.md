...
`archive` — это ближайший аналог «удаления» для НЕ-черновиковой кампании
«Мастера кампаний»: живая разведка (issue #633) подтвердила, что
отдельного действия «удалить» в интерфейсе Яндекса для неё **нет** — только
«Архивировать» в меню страницы обзора. `masters archive` кликает по этому
пункту меню и проверяет через список кампаний, что кампания реально стала
архивной, прежде чем сообщить об успехе. Команда идемпотентна (уже архивная
кампания — предупреждение, а не ошибка), но **необратима из этого CLI** —
команды `masters unarchive` нет. Тот же отказ от `--sandbox`, что и у
suspend/resume выше.

`delete` (issue #782) удаляет кампанию именно в статусе **DRAFT**
(«Черновик») — единственном статусе, для которого `archive` не работает,
так как у страницы обзора черновика вообще нет меню «⋮» (issue #660).
Черновик, созданный по ошибке (например через `masters add --draft`), до
этого был дорогой в один конец: единственный документированный выход —
запустить его, потратив реальные деньги, только чтобы получить доступ к
архивации. Живая разведка нашла, что собственное меню строки в ГРИДЕ
кампаний (отдельное от меню страницы обзора) предлагает «Удалить» для
строки-черновика — `masters delete` использует именно его. **Отказывает
для любого статуса, кроме DRAFT**, с понятной ошибкой, отсылающей к
`masters archive`. В отличие от всех остальных мутаций `masters`, Яндекс
**не показывает диалог подтверждения** перед удалением — кампания исчезает
сразу после клика — поэтому эта команда всегда сама запрашивает
подтверждение: по умолчанию интерактивно, либо `--yes` для пропуска
запроса в неинтерактивном сценарии. Тот же отказ от `--sandbox`, что и у
suspend/resume выше.
