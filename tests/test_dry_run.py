...
# ----------------------------------------------------------------------
# campaigns add: SmartCampaign.BiddingStrategy.Network families (#368)
#
# Source of truth: cached SOAP WSDL ``tests/wsdl_cache/campaigns.xml``
# (Yandex public docs at yandex.ru/dev/direct/doc/* are blocked by
# showcaptcha and return HTTP 404 to scripted fetches; the cached WSDL
# is the authoritative reference for the Network branch of
# SmartCampaign.BiddingStrategy).
#
# WSDL ref: campaigns.xml lines
#   * 411-426 — ``SmartCampaignNetworkStrategyTypeEnum`` (NETWORK_DEFAULT
#     + 9 Per-* / ROI / CRR families + SERVING_OFF + UNKNOWN sentinel).
#   * 1401-1481 — shared ``Strategy*Add`` subtypes (identical to the
#     Search branch — issue #367/#385).
#   * 1510-1514 — ``StrategyNetworkDefaultAdd`` (single optional
#     ``LimitPercent`` ``xsd:int``).
#   * 1789-1810 / 1822-1834 — ``SmartCampaignStrategyAddBase`` /
#     ``SmartCampaignNetworkStrategyAdd`` containers.
#   * 1875-1882 — ``SmartCampaignStrategyAdd`` envelope (``Search`` /
#     ``Network`` both ``minOccurs=1`` on add).
#   * 1965-1978 — shared ``CustomPeriodBudget`` / ``ExplorationBudget``
#     complex types (all-or-nothing groups).
#   * 858-929 — get-side ``Strategy*`` types (carry ``BudgetType`` —
#     update-only, mirrors ``--smart-search-budget-type``).
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# campaigns update: SmartCampaign.BiddingStrategy.Network families (#368)
# ----------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# #364: TextCampaign.BiddingStrategy.Network typed-flag dry-run coverage.
# WSDL reference: ``TextCampaignNetworkStrategyAdd`` (campaigns.xml 1609) +
# ``TextCampaignStrategyAddBase`` (1581) + per-subtype ``Strategy*Add``
# complex types (1339-1514).
# ---------------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# adgroups.get: separate per-subtype *AdGroupFieldNames (issue #405)
# ----------------------------------------------------------------------
...
_ADGROUPS_GET_NESTED_FIELD_FLAGS = [
    ...
    (
        ...
        "Source,FeedId,SourceType",
    ),
    (
        "--mobile-app-ad-group-field-names",
        "MobileAppAdGroupFieldNames",
        "StoreUrl,TargetDeviceType,AppOperatingSystemType",
    ),
    (
...
def test_adgroups_get_nested_field_names_payload():
    ...
    # covering per-subtype projections (DynamicTextAdGroup, MobileAppAdGroup,
    # SmartAdGroup, UnifiedAdGroup, TextAdGroupFeedParams) and per-feature
    # AutotargetingSettings.BrandOptions/Categories enums shared with
    # keywords.get (#413).
    argv = ["adgroups", "get"]
    expected = {}
...
def test_adgroups_get_omits_nested_field_names_by_default():
    body = _read_dry_run("adgroups", "get")
