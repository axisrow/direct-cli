...
| #56 | Live API draft campaign lifecycle (opt-in) | `supported` | `tests/test_v5_live_write.py` + cassette `test_v5_live_draft_campaign_create_get_delete.yaml`; guarded by `YANDEX_DIRECT_LIVE_WRITE=1`; mirrored as `TestWriteCampaignDraftLifecycle` in sandbox suite | Resolved: live-write root fixture in place; follow-up live-write expansion tracked as separate future issues |
...
| #69 | `adgroups.suspend/resume`, `audiencetargets.update`, `dynamicads.update`, `dynamicfeedadtargets`, `bidmodifiers.toggle` | mixed: unsupported methods + one transport mapping gap | Live API routing returned error 55 for unsupported methods; WSDL confirms no `adgroups.suspend/resume`, no `audiencetargets.update`, no `dynamictextadtargets.update`, and no `bidmodifiers.toggle`; `dynamicfeedadtargets` exists in live API/WSDL and was missing from the `axisrow/tapi-yandex-direct` transport mapping | Do not add unsupported CLI commands; fix `axisrow/tapi-yandex-direct` mapping/extraction for `dynamicfeedadtargets`, pin direct-cli to the fixed fork revision, and close after regression coverage. Superseded for `adgroups.suspend/resume` specifically by #573 below — see that row for why this is no longer a blanket "don't add" |
| #573 | `adgroups suspend/resume` (CLI-only helper, not `adgroups.suspend`/`adgroups.resume` RPC methods) | `not-in-wsdl` for AdGroups, `supported` via `ads.*` | #69's finding that AdGroups has no `suspend`/`resume` RPC method still holds — confirmed again against WSDL. But #69's "do not add" was about calling those nonexistent methods directly (error 55). #573 instead composes two methods that ARE supported (`ads.get` + `ads.suspend`/`ads.resume`, both `WRITE_SANDBOX`) to emulate "pause the group" the way the web UI/Direct Commander do | Added `adgroups suspend`/`adgroups resume` as CLI-only helpers, registered in `INTENTIONAL_EXTRA_METHODS` (`direct_cli/wsdl_coverage.py`) since they are not 1:1 WSDL mirrors; classified `WRITE_SANDBOX` (not `DANGEROUS`) to match the underlying `ads.*` methods |
